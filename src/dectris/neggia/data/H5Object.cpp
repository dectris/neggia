...
{
   if(_offset == H5_INVALID_ADDRESS) throw std::out_of_range("object pointing to invalid address");
...
void H5Object::debugPrint(int nBytes) const
{
   ...
   for(int i=0; i<nBytes; ++i) {
      std::cout << std::hex << std::setfill('0') << std::setw(2) << (unsigned int)(*(unsigned char*)(_fileAddress + _offset+i)) << " ";
      if((i+1)%4 == 0) {
